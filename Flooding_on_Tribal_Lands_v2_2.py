...
# ## A Common Issue Spring of 2019 Tribal Nations in South Dakota suffered from extreme flooding that resulted in
# road loss, land degradation, and property damage. Most reservations do not have funding or resources to replace or
# rebuild. In the case of road loss, sometimes that road may be the only route causing the possibility of communities
# or families to be isolated from resources. Property damage for farmers and ranchers can take years to recover,
# sometimes resulting in total loss.
# 
# Creating a reservation wide map that contains high to low risk levels will help the tribe identify areas to design
# preventative measures rather than reacting to the situation. Additionally, landowners can see what parts of their
# property are at risk, allowing them to use different mitigation strategies.
# 
# These reasons indicate why it is important for our team to create a universal code that results in a flooding risk
# map for utilization by all Tribal Entities. We hope the risk map created by the code will help to minimize the
# issues of land erosion, road and land loss, property damage, land degradation and loss of sacred sites within each
# reservation.
# 
# 
# 

# ## Prior Work There are several organizations that have created where flood maps for most places of the United
# States, but their intended use is for insurance. One such organization is [FEMA]( https://www.fema.gov/flood-maps),
# who creates these maps as part of the National Flood Insurance Program (NFIP). Unfortunately when looking at the
# maps, the information is not very useful to the average person.
# 
# Another option was with the US Geological Survey (USGS) when they created flood maps that showed the extent of
# flooding for different flood severities. Although these maps are more intuitive to understand, they are also very
# hard to find and, in many cases, hard copies are only located in libraries of Universities. The USGS does have some
# very useful [flood information]( https://www.usgs.gov/faqs/where-can-i-find-flood-maps?qt-news_science_products=0
# #qt-news_science_products) online, but it is limited in scope.
# 
# 

# ## The Method Our team derived it’s method from a similar analysis that was used to create an island wide
# inundation vulnerability map for the island of St. Lucia, located on the edge of the Caribbean Sea north of
# Venezuela. The distinctive variables (Land use, Soils, Precipitation, and a Digital Elevation Model) implemented in
# the document generate risk levels associated with flooding.
# 
# To determine the infiltration rate, soil type and land-use identify the Runoff Curve Numbers (CN). These numbers
# range from 30 to 100, with 30 being the lowest and 100 having the highest potential for runoff. Land-use
# categorizes CNs by soil hydrologic units (A-D) so that intersecting soil and land-use layers identify the correct
# CN. Using precipitation data, daily amounts for mean annual rainfall uses only data for extreme precipitation
# events because they result in flooding.
# 
# Finally, horizontal slopes have the capacity for pooling, thus contributing more to flooding. To compensate,
# slope has a risk levels of one, two, and six instead of three for horizontal gradients are. After determining risk
# levels of each variable, the summing of the overlapping values to identify the final risk level results in one
# through six being low risk and anything greater than nine being high risk.
# 
# (insert figure 1)
# 
# Flow Chart from St. Lucia Document

# ## Conclusion As is everyone’s goal when setting out to complete a project, not all come to fruition. In the case
# of our code, it is about 80% complete due to time constraints resulting in the exclusion of precipitation data. One
# of the big reasons why precipitation data was not included at this time is due to identifying the correct
# interpolation method and then implementing. Many reservations have very few rain gauges that continuously collect
# data. For Standing Rock Reservation in particular there are only three gauges on the reservation.
# 
# Despite precipitation’s absence, the results with just Slope, Soil, and Land-use are rather interesting. When
# looking at the intermediate layer of CN Risk, calculated from Soil and Land-use, it would appear that the results
# are the opposite of expectations. With one indicating low risk and three as high risk, the resulting map was
# inverse of the expected results.
# 
# (insert Figure 2)
# 
# The most likely cause stems from CN value assignments made to the Land-use layer. The information that we were
# working with did not contain a complete array of CN values for all Land-use types. Instead, the data’s focus was
# agricultural lands. Additionally the St. Lucia document’s study sight was an island so there is also reason to
# believe that the reassignment of CN value to Risk Level may need alterations for an inland location. Instead of CN
# values of less than 40 qualifying for low risk, inland might receive more accurate results with CN values less than
# 50 qualifying for low risk. We believe the same issue exists for slope data, thus we increased the ranges used in
# the St. Lucia paper by a multiple of 10.
# 
# Overall, we are proud of the code that we created as an initial base for the analysis. We believe that the
# foundation created will enable the continuation of building a code that will be able to take user inputs and export
# a final map with eases. We cannot guarantee that those who will use the code will have a python background,
# which is why one of our goals was user ease. Down the line, the incorporation of a simple user interface for the
...
# ### Documents 1. Flood Hazard Mapping of St. Lucia (Document) - By: Vincent Cooper and Jacob Opadeyi (February
# 2006) - Chukwuka, Azubuike. (2013). Re: Methodology to "generate flood risk maps".. Retrieved from: -
# https://www.researchgate.net/post/Methodology_to_generate_flood_risk_maps/51abca3ed3df3e257b000065/citation
# /download. 2. Hydrologic Soil-Cover Complexes (Document) - USDA-NRCS - NEH, Part 650, (EFH), Amend, IA50, NOV,
# 2007 - https://www.nrcs.usda.gov/Internet/FSE_DOCUMENTS/nrcs142p2_011485.pdf 3. Additional Runoff Curve Number
# Information (Website) - https://en.wikipedia.org/wiki/Runoff_curve_number

# ### Data Sources 1. Reservation Boundary - Originator: Branch of Geospatial Support - Data Name: Land Area
# Representations (LAR) - Download format: Zip File with Shapefile(s) (.shp) - Website: [US Department of the
# Interior - Indian Affairs](biamaps.doi.gov/bogs/datadownload.html) - Publication Date: 2018-12-10 2. Land-Use Layer
# - Originator: U.S. Geological Survey Gap Analysis Program - Data Name: GAP/LANDFIRE National Terrestrial Ecosystems
# 2011 - Download format: Zip File with Raster(s) (.tif) - Website: [USGS - Gap Analysis Project](
# https://www.usgs.gov/core-science-systems/science-analytics-and-synthesis/gap/science/land-cover-data-download?qt
# -science_center_objects=0#qt-science_center_objects) - Publication Date: 2016-05-13 3. Soil Layer - Originator:
# U.S. Department of Agriculture, Natural Resources Conservation Service - Data Name: Soil Survey Geographic (SSURGO)
# database - Download format: Zip File with Shapefile(s) (.shp) - Website: [USDA NRCS Web Soil Survey](
# https://websoilsurvey.sc.egov.usda.gov/) - Publication Date: 2019-09-16 4. Slope Layer - Originator: USDA/NRCS -
# National Geospatial Center of Excellence - Data Name: National Elevation Dataset 30 meter - Download format: Zip
# File with Raster(s) (.tif) - Website: [USDA NRCS Geospatial Data Gateway](https://gdg.sc.egov.usda.gov/) -
...
# 
# These are the different tools to import for the code to run. If you do not already have to package please install.
# Much of the code can be found at the [GitHub Earth Lab Repository](
# https://github.com/earthlab/earth-analytics-python-env). The list of packages associated from the repository can be
# found [here](https://github.com/earthlab/earth-analytics-python-env/blob/master/environment.yml).
# 
# The only package used and not found in the Earth Lab Repository is [RichDEM](
# https://richdem.readthedocs.io/en/latest/). This package is used to create a slope layer from the DEM files.
...
# [return to Preperation](#Preperation)
# 
# User should confirm that the vairable's folder and file names match those on their system. Vairables are grouped by
# the following: * Folders [go](#Folder-Vairables) * Specified Original Data [go](#Specific-Originial-Data-Files) *
# Saved Data Variables [go](#Saved-Data-Variables) * Set Values [go](#Set-Values) * Folder Check Codes [go](
# #Folder-Check-Codes)

# #### Folder Vairables
# [return to 2. Vairables](#2.-Vairables)
# 
# These vairables contain the pathways for the code to navigate to folders where either data will be collected or
# saved. Please only change the red text to fit your folder names. In general each layer type has two folders,
# an original data folder and a results folder. Only layers that are rasters also have a projection folder. This is
# to help reduce the risk of confusion for the code.
# 
# The final layer created of each vairable is then saved in the Risk Layer folder's input folder. This way when
# looking back on files it is easier to identify which layers were inputs vs. outputs. Any layer that is the result
# of layers being combined is saved in the Risk Layer folder's output folder.
# 
# Next there are three additional folders: - preped_csv - The location for CSV tables to be saved for joinig with
# decitated vairable. Go to [Specific Originial Data](#Specific-Originial-Data-Files) for the particulars of setting
# up the CSVs. - map_layers - These layers are cities, road, streams, and rivers that contains the area which is used
# for the creation of the final map. - saved_figs - This is where all eight figures will be saved.
# 
...
# 
# Finally, folders do not need to be created in advance [(see Folder Check Codes)](#Folder-Check-Codes). As long as
# the parent folder is set to where you want everything to be pulled from and saved to it will create the folders.
# The code will end if it creates any of the folders that require original data to be held in the folder. If it does
# please move the approperate data to the folders listed as created.
# 
...
# [return to 2. Vairables](#2.-Vairables)
# 
# These vairables are for specific files to access. Please only change full file names. If selecting a specific shape
# file the extension is ".shp". **Make sure file extensions are Present!**
# 
# ##### Special instructions for CSV files
# Soil Table: 
# 
# When opening the Muaggatt table in a spreadsheet program it will be a text file that is | seperated (that symbol is
# on the same keyborad buttion as \\ ). All of the headers are missing but if you go to the [SSURGO Table Column
# Descriptions](https://data.nal.usda.gov/system/files/SSURGO_Metadata_-_Table_Column_Descriptions.pdf) and look for
# Muaggatt, all of the headers are there in order. Just add a row at the top of the sheet and copy/paste the Column
# Physical Name into the first cell of each column. **Do not use the Column Label.** Make sure to save the file as a
# CSV.
# 
# Land-use Table:
# 
# To prepare the Land-use table there are three steps (starting on step 3 the formulas are for use in excel): 1.
# First create a spreadsheet with headders of CN_Join, Description, Sub_Description, Hydrologic_Condition, CN-A,
# CN-B, CN-C, CN-D. To fill in each column all fields, except the first, are derrived from the 2nd and 3rd item found
# in [Documnents](#Documents). The first column just runs throught the alphabet and will be used later. 2. Each
# Land-use layer contains two necessary files to create a new one. - The first is a text file that ends in
# Attributes. Save a copy of this file for manipulation. - The second is one that ends with .tif.vat.dbf. The data
# from this file must be coppied into a new spreadsheet. If more than one Land-Use layer is to be used copy the data
# from each into one new spreadsheet. 3. By now there should be a total of 3 spreadsheets. (CN values, Attributes,
# coppied dbf) In the attribute file in the very last column give it a headder of "Present" and then insert the
# following code in to all of the cells: - `=IF(IFERROR(VLOOKUP($A2,dbf_file.csv!$A$1:$C$(last_row_number),1,FALSE),
# "No")="No","No","Yes")` 4. Delete every row that has a "No" under Present. This can be acceved easily if the table
# is sorted by the Present column and scroll down until the boundry between "Yes" and "No" is found or filter out the
# "Yes". Then highlight the group of cells that say "No" and delete. 5. This next part is a bit tedious but create a
# new column and give each description a CN_Join letter from the first table created. Hopefully in future versions of
# this code, this part can be automated. 6. Add for new columns (CN-A, CN-B, CN-C, CN-D) and in each column us the
# code below updating the \*number\* with the correct number for it's column name. - `=VLOOKUP($V2,
# Hydrologic_SoilCover_Complexes_CN.csv!$A$1:$H$(last_row_number),*number*,FALSE)` - Replace the \*number\* with one
# of the following numbers that matches it's column header: - 5 = CN-A - 6 = CN-B - 7 = CN-C - 8 = CN-D 7. Copy and
# paste all values to remove formulas and save as a .csv
# 
...
# 
# **Note:** The projection vairable is set to use ESPG values.Insert the correct number directly after the colon. Use
# this site to determin ESPG number: https://spatialreference.org/
...
# [return to 2. Vairables](#2.-Vairables)
# 
# These two blocks of code are used to check that all the folders are in order. Having the folders in place is not
# necessary, but will prevent the code from stopping if any of the folders that hold original data are missing.
# 
# Please make sure to have the correct data in the correct folder, no subfolders within, for the original data. As a
# reminder the [original data folder](#Folder-Vairables) vairables are as follows with their needed file types: *
# landuse_original_folder - All raster files that cover the area * soil_original_folder - All shapefiles that cover
# the area * slope_original_folder - All DEMs that cover the area * reservation_boundry_original_folder - A shapefile
# * preped_csv - Must contain the soil table and landuse table * map_layers - Shapefiles for cities, road, streams,
# and rivers (1 each)
...
if len(need_data) == 0:
    ...
    sys.exit(
        "One or more original data folder was created.\nPlease move original data to the approperate folders and then "
        "restart")
...
# 
# An additional Temp folder and files will be created due to the time status tracking. Everytime the code is run the
# time laps from the start time will be added to a file that has the time stamp from when the code was initiated. The
# most recent file is a great place to see what the status of the code is at.
...
def time_status(description):
    ...
    with open(timefile, 'a') as textfile:
        textfile.write(timestamp + "," + description + "," + str(complete_time) + "\n")
    if complete_time / 60 < 1:
        print(complete_time, " Seconds have elapsed - " + description + "\nTimestamp: " + timestamp)
    elif complete_time / 3600 < 1:
        print(complete_time / 60, " Minutes have elapsed - " + description + "\nTimestamp: " + timestamp)
    else:
        print(complete_time / 3600, " Hours have elapsed - " + description + "\nTimestamp: " + timestamp)
...
# 
# This set of code isolates a particular reservation from the BIA shapefile and updates the projection to match the
# indicated ESPG value.
...
# [return to Analysis](#Analysis)
# 
# Using the St. Lucia document as a guide, this set of code combins the Land-use and Soil layers to create the
# CN_risk layer. Both layers are intersected and based on Hydrologic Code the approperate CN value from the Land-use
# iss assigned to the polygon. CN Risk Value is then assigned based on CN range.
...
# [return to Analysis](#Analysis)
# 
# Using the St. Lucia document as a guide, this set of code combines CN Risk and Slope. Both layers are intersected
# and then the sum of their risk values is totaled. Flood Risk is then assigned based on Total Risk range.
